...
export const loadLearningData=()=>Promise.all([load<LearningContent>('/data/learning-content.json'),load<CoverageLedger>('/data/coverage-ledger.json','1.1.0'),fetch('/data/referenced-roads.geojson').then(r=>r.json())])
export const loadRoadData=()=>Promise.all([
  load<RoadTopology>('/data/road-topology.json'),
  load<RoadGeometryCollection>('/data/road-network.geojson'),
])
export const loadRoadNetwork=()=>load<RoadGeometryCollection>('/data/road-network.geojson')
